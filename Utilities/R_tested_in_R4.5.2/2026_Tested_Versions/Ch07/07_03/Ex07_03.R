...
?sleep
sleep[1:5, ]
sld <- sleep[, 1:2]  # Save just the first two variables
sld[1:5, ]  # Show the first 5 cases

# Some quick plots to check data
hist(sld$extra, col = "lightgray")
boxplot(extra ~ group, data = sld)

# Independent 2-group t-test (with defaults)
t.test(extra ~ group, data = sld)
