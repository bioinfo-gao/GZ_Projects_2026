...
brain_spatial <- Load10X_Spatial(data.dir = data_dir, 
...
brain_spatial <- PercentageFeatureSet(brain_spatial, "^mt-", col.name = "percent.mt")
brain_spatial <- subset(brain_spatial, subset = nFeature_Spatial > 200 & percent.mt < 20)
print(dim(brain_spatial))
...
brain_spatial <- SCTransform(brain_spatial, 
                             assay = "Spatial", 
...
# 3. 检查零表达比例 (极重要)
zero_count_percent <- sum(LayerData(brain_spatial, layer = "counts") == 0) / (19652 * 2312) * 100
...
# 将图谱保存到服务器
ggsave("visium_analysis_results.pdf", plot = p1 + p2 / p3, width = 15, height = 10)
